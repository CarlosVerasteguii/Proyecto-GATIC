...
class CreableSelectorsScopeTest extends TestCase
{
    ...
    public function test_contracts_index_filters_do_not_render_creable_supplier_cta(): void
    {
        $admin = User::factory()->create(['role' => UserRole::Admin]);

        Livewire::actingAs($admin)
            ->test(ContractsIndex::class)
            ->assertSeeHtml('id="contracts-supplier-filter"')
            ->assertDontSee('supplier-option-create-');
    }
}
